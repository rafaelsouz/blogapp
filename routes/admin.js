...
router.post("/categorias/nova", eAdmin, (req, res) => {
    ...
    if (req.body.nome.length < 3) {
        erros.push({texto:'nome da categoria muito pequeno, no minimo 3 caracteres'})
    }
    ...
    if (erros.length > 0 ) {
        ...
        new Categoria(novaCategoria).save().then(()=>{
            ...
            console.log('error ao salvar categoria'+ err);
            res.redirect('/admin')
        })
    }
})
            ...
            if (erros.length > 0 ) {
                ...
                Categoria.findOne({_id: req.body.id}).then((categoria)=>{
                    ...
                    categoria.save().then(()=>{
                        ...
                        req.flash('error_msg','Houve um error ao salvar a edição da categoria')
    ...
    router.post('/postagem/nova', eAdmin, (req,res)=>{
        ...
        if (req.body.categoria == '0'){
            erros.push({texto:'Categoria invalida, regristre uma categoria'})
    ...
    router.post('/postagem/edit', eAdmin, (req,res)=>[
        Postagem.findOne({_id:req.body.id}).then((postagem)=>{
            ...
            postagem.save().then(()=>{
                ...
                req.flash('error_msg', 'Houve um error ao salvar a edição da postagem')
                res.redirect('/admin/postagens')

            })
        }).catch((err)=>{
            req.flash('error_msg','Houve um error ao salvar a ediçao')
            res.redirect('/admin/postagens')
        })
    ])
